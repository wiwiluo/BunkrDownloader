...
class MediaDownloader:
    ...
    def skip_file_download(self, final_path: str) -> bool:
        ...
        def log_skip_event(reason: str) -> bool:
            ...
            self.live_manager.update_task(
                self.download_info.task, completed=100, visible=False,
            )
            return True
    ...
    def handle_partial_download(self) -> None:
        ...
        self.live_manager.update_log(
            "Partial download",
            f"The extension of {self.download_info.filename} has been modified to "
            "'.temp' because of empty data blocks. Check the log file.",
        )
    ...
    def handle_request_exception(self, req_err: RequestException, attempt: int) -> bool:
        ...
        if req_err.response.status_code in (429, 503):
            self.live_manager.update_log(
                "Too many requests",
                f"Retrying to download {self.download_info.filename}... "
